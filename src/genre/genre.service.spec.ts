...
describe('GenreService', () => {
  ...
  describe('create', () => {
    it('should create a genre successfully', async () => {
      ...
      expect(repository.save).toHaveBeenLastCalledWith(createGenreDto);
      expect(result).toEqual(savedGenre);
    });

    it('should return BadRequestException if given already exist genre', async () => {
      const createGenreDto = { name: 'Fantasy' };
      
      jest.spyOn(repository, 'findOne').mockResolvedValue({ id: 1, ...createGenreDto } as Genre);

      expect(service.create(createGenreDto)).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('findOne', () => {
    it('should return a genre if found', async () => {
      ...
      expect(repository.findOne).toHaveBeenLastCalledWith({ where: { id: genre.id }});
  ...
  describe('update', () => {
    it('should update and return the genre if it exist', async () => {
      const updateGenreDto = { name: 'Updated Fantasy'};
  ...
  describe('remove', () => {
    ...
    it('should throw a NotFoundException if genre to delete does not exits', async () => {
      jest.spyOn(repository, 'findOne').mockResolvedValue(null);
