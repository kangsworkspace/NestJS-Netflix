...
describe('UserController', () => {
  ...
  describe("create", () => {
    ...
    it('should return a list of users', async () => {
      const users = [
        { id: 1, email: 'test@codefactory.ai'},
        { id: 2, email: 'test@codefactory2.ai'},
      ]
  ...
  describe("update", () => {
    it('should return the updated user', async () => {
      const id = 1;
      const upadteUserDto: UpdateUserDto = {
      ...
      const result = await userController.update(id, upadteUserDto);

      expect(userService.update).toHaveBeenCalledWith(id, upadteUserDto);
      expect(result).toEqual(user);
    });
  });
