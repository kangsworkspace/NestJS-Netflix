...
describe('MovieService', () => {
  ...
  let commmonService: jest.Mocked<CommonService>;
  ...
  beforeEach(async () => {
    ...
    commmonService = unitRef.get(CommonService);
  ...
  describe('findRecent', () => {
    it('should return recent movies form cache', async () => {
      const cachedMovies = { id: 1, titile: 'Movie1' };
    ...
    it('should fetch recent movies form  the repository and cache them if not found in cache', async () => {
  ...
  describe('findAll', () => {
    ...
    it('should return a list of movies without user likes', async () => {
      const moives = [{ id: 1, title: 'Movie 1' }];
      const dto = { title: 'Movie' } as GetMoviesDto;
      const qb: any = {
        where: jest.fn().mockReturnThis(),
        getManyAndCount: jest.fn().mockResolvedValue([moives, 1]),
      };

      getMoviesMock.mockResolvedValue(qb);
      jest.spyOn(commmonService, 'applyCursorPaginationParamsToQb').mockResolvedValue({ nextCursor: null } as any);
      ...
      expect(commmonService.applyCursorPaginationParamsToQb).toHaveBeenCalledWith(qb, dto);
    ...
    it('should rerturn a list of movies with user likes', async () => {
      const movies = [
        { id: 1, title: 'Movie 1' },
        { id: 3, title: 'Movie 3' },
      ];
      ...
      getMoviesMock.mockResolvedValue(qb);
      jest.spyOn(commmonService, 'applyCursorPaginationParamsToQb').mockReturnValue({ nextCursor: null } as any);
    ...
    it('should return movies without title filter', async () => {
      ...
      const qb: any = { getManyAndCount: jest.fn().mockResolvedValue([movies, 1]) };
  ...
  describe('create', () => {
    ...
    it('should throw NotFoundException if director does not exist', async () => {
      ...
      await expect(movieService.create(createMovieDto, userId, qr))
        .rejects
  ...
  describe('update', () => {
    ...
    it('should update a movie successfully', async () => {
      ...
      expect(qr.commitTransaction).toHaveBeenCalledWith();
      expect(result).toEqual(movie);
    });
    ...
    it('should throw NotFoundException if new director does not exists', async () => {
      const updateMovieDto: UpdateMovieDto = {
        title: 'Updated Movie',
        directorId: 1,
      };
  ...
  describe('toggleMovieLike', () => {
    ...
    it('should delete like record when isLike is the same as the existing record', async () => {
      ...
      expect(findOneMovieMock).toHaveBeenCalledWith({ where: { id: 1 } });
      expect(findOneUserMock).toHaveBeenCalledWith({ where: { id: 1 } });
      expect(getLikedRecordMock).toHaveBeenLastCalledWith(1, 1);
    ...
    it('should throw BadRequestException if movie does not exist', async () => {
      ...
      expect(findOneMovieMock).toHaveBeenCalledWith({ where: { id: 1 }});
      expect(findOneUserMock).not.toHaveBeenCalledWith({ where: { id: 1 }});
    });

      it('should throw UnauthoriedException if movie does not exist', async () => {
      const movie = { id: 1 };
