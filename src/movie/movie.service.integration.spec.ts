...
describe('MovieServie - Integration Test', () => {
      ...
      imports: [
        ...
        TypeOrmModule.forRootAsync({
          useFactory: (configService: ConfigService) => ({
            ...
            database: configService.get<string>(envVariableKeys.dbTestDatabase),

            entities: [Movie, MovieDetail, User, Director, Genre, MovieUserLike],
            synchronize: true,
            logging: false,
          }),
          inject: [ConfigService],
        }),
        TypeOrmModule.forFeature([Movie, MovieDetail, User, Director, Genre, MovieUserLike]),
      ],
      providers: [
        MovieService,
        CommonService,
  ...
  describe('findRecent', () => {
    ...
    it('should cache recent movies', async () => {
      ...
      const cahcedData = await cacheManager.get('MOVIE_RECENT');

      expect(cahcedData).toEqual(result);
    });
  });
  ...
  describe('remove', () => {
    it('should remove movie correctly', async () => {
      const removieId = movies[0].id;

      const result = await service.remove(removieId);

      expect(result).toBe(removieId);
    });
